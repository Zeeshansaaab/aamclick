...
class WithdrawalController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $withdrawals = Withdrawal::where('user_id', $withdrawal->user->id)->where('status', '!=', 0)->with(['user', 'method'])->orderByDesc('id')->get();
        $pageTitle = $withdrawal->user->aam_id.' Withdraw Requested ' . showAmount($withdrawal->amount) . ' '.$general->cur_text;
        $details = $withdrawal->withdraw_information ? json_encode($withdrawal->withdraw_information) : null;

        return view('admin.withdraw.detail', compact('pageTitle', 'withdrawal','details','withdrawals'));
    }
    ...
    public function reject(Request $request)
    {
        ...
        $user = $withdraw->user;
        if($withdraw->type == 'profit_bonus')
            $user->profit_bonus += $withdraw->amount;
        elseif($withdraw->type == 'user_profit_bonus')
            $user->user_profit_bonus += $withdraw->amount;
        elseif($withdraw->type == 'deposit_commission')
            $user->deposit_commission += $withdraw->amount;
        else

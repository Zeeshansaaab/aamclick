...
class DepositController extends Controller
{
    ...
    public function details($id)
    {
        ...
        $deposits = Deposit::where('user_id', $deposit->user->id)->where('status', '!=', 0)->with(['user', 'gateway'])->orderByDesc('id')->get();
        $pageTitle = $deposit->user->aam_id.' requested ' . showAmount($deposit->amount) . ' '.$general->cur_text;
        $details = ($deposit->detail != null) ? json_encode($deposit->detail) : null;
        return view('admin.deposit.detail', compact('pageTitle', 'deposit', 'deposits','details'));
    }
    ...
    public function approve($id)
    {
        ...
        // $addProfitToUser = new AddProfitToUser();
        // $addProfitToUser->user_id = $deposit->user_id;
        // $addProfitToUser->amount = $deposit->amount;
        // $addProfitToUser->deposit_id = $deposit->id;
        // $addProfitToUser->deposit_date = Carbon::now();
        // $addProfitToUser->status = 0;
        // $addProfitToUser->save();


        levelCommission($user, $deposit->amount, 'deposit_commission', $deposit->trx,$user->plan->id);

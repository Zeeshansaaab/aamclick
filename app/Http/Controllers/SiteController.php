...
class SiteController extends Controller
{
    public function index(){
        ...
        $cryptos = CryptoPrice::select('symbol','name','usd','change_24_hr','change_7_day')->get();
        $calendars = Calendar::select('title' , 'start')->get();
        return view($this->activeTemplate . 'home', compact('pageTitle','sections','cryptos','calendars'));
    }
    ...
    public function committeePlans()
    {
        $pageTitle = 'Committee Plans';
        $plans = CommitteePlan::where('status',1)->get();
        return view($this->activeTemplate.'committee.committee_plans', compact('pageTitle','plans'));
    }

    public function committees($planid)
    {
        $committeePlan = CommitteePlan::findOrFail($planid);

        $data = [
            'pageTitle' => 'Committees',
            'committeeLists' => Committee::where('committee_plan_id', $committeePlan->id)->where('status',1)->orderBy('id','desc')->get(),
            'committeePlan' => $committeePlan
        ];

        return view($this->activeTemplate.'committee.committees_list', $data);
    }

                    ...
                    <div class="card-body">
                        <form action="{{route('user.withdraw.money')}}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-select form--select margin-top-10" name="type" required>
                                    <option value="">@lang('Select Type')</option>
                                    @foreach($withdrawType as $data)
                                        <option @if($data->can_withdraw_in !=0 && \Carbon\Carbon::today()->day % $data->can_withdraw_in != 0 ) disabled @endif
                                         value="{{ $data->typekey }}" data-resource="{{$data}}"> {{__($data->type)}} {{$data->can_withdraw_in ==0 ? '(Any Time)' : '('.$data->can_withdraw_in.')'}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">

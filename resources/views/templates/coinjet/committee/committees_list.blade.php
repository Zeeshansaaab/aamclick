...
@section('content')
    <div class="container container-plan">
        ...
        <div class="row">
            @foreach($committeeLists as $committee)
            <div class="col-md-3 mt-3 col-sm-6">
						<div class="card-premium px-3">
							<div class="card-header @if($loop->index == 0) header-basic @elseif($loop->index==1) header-standard @elseif($loop->index==2) header-premium @endif">
								<h3>{{$committee->name}}</h3>
							</div>
							<div class="card-body">
								<p>
								<h3>{{ $general->cur_sym . getAmount($committee->amount) }}</h3>
								</p>
								<div class="card-element-hidden-premium">
                                    <ul>
                                        <li class="font-weight-bold text-success"><span class="fs1" aria-hidden="true" data-icon="N"></span> Validity : {{ $committee->validity }} Months</li>
                                        <li class="border-l font-weight-bold"><span class="fs1" aria-hidden="true"></span>Amount will be return in {{ $committee->amount_return }} Months</li>
                                        <li><p class="fs1" style="margin-left:20px;" aria-hidden="true" data-icon="N">Current Members: @if($committee->members < $committee->validity) {{ $committee->members }} Members @else Full @endif</p></li>
                                    </ul>

                                    @if($committee->members < $committee->validity)
                                        <a href="{{ route('user.committeeMembers', [$committeePlan->id, $committee->id]) }}" class="btn btn-primary">Get Detail</a>
                                    @else
                                        <span class="btn btn-primary disabled">Get Details</span>
                                    @endif
								</div>
							</div>
						</div>
